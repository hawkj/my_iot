...
func Bme280(ctx context.Context, g *common.Global) {
	...
	for {
		...
		mqttMessage.MsgType = commoncons.MqttMsgTypeDeviceData
		mqttMessage.Data = data
		jsonData, err := json.Marshal(mqttMessage)
		if err != nil {
			log.Println(err)
			continue
		}

		//if g.Config.AppEnv.Name == "test" {
		//	log.Println("SendMessage: ", jsonData)
		//}
